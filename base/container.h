...
{
public:
	int	operator()(const T& data) const
	{
...
public:
	...
	struct entry
	{
		...
		int	m_hash_value;		// avoids recomputing.  Worthwhile?
		T	first;
		U	second;
...
private:
	...
	static const int TOMBSTONE_HASH = (int) -1;
	...
	int compute_hash(const T& key) const {
		int hash_value = hash_functor()(key);
...
template<class U>
class stringi_hash : public hash<tu_string, U, stringi_hash_functor<tu_string> >
{
};
*/
