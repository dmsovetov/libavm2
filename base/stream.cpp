// avm2_stream.cpp	-- Thatcher Ulrich <tu@tulrich.com> 2003
...
//#include "avm2/avm2_log.h"
//#include "avm2/avm2_types.h"
#include "tu_file.h"
#include <string.h>
...
//		IF_DEBUG(log_msg("val = 0x%X\n", val));
    ...
    int stream::readU8( Sint32& result, const Uint8* input )
    {
        result = *input;
        return 1;
    }

    int stream::readU30( Sint32& result, const Uint8* input )
    {
        result = input[0];
        ...
        result = (result & 0x0000007F) | input[1] << 7;
        if( (result & 0x00004000) == 0 ) {
            return 2;
        }

        result = (result & 0x00003FFF) | input[2] << 14;
        if( (result & 0x00200000) == 0 ) {
            return 3;
        }

        result = (result & 0x001FFFFF) | input[3] << 21;
        if( (result & 0x10000000) == 0 ) {
            return 4;
        }
        
        result = (result & 0x0FFFFFFF) | input[4] << 28;
        return 5;
    }

    int stream::readS24( Sint32& result, const Uint8* input )
    {
        result = input[0] | (input[1] << 8) | ((char)input[2] << 16);
        return 3;
    }
